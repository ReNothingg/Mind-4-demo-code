...
![CI status](https://github.com/ReNothingg/Mind-4-demo-code/actions/workflows/ci.yml/badge.svg)

> Если вы не знаете половину слов, лучше просто посмотреть код и не зарускать его. *Не надо мучать свои комплектуюшие...*

Mind 4 — лаборатория для тех, кто хочет понять, как работает +- генеративная модель, и попробовать её «вживую» — запустить простой чат и экспериментирвать с ядрами на GPU.
