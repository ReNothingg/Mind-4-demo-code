...
PyMODINIT_FUNC PyInit__metal(void)
{
    ...
    if (PyType_Ready(&PymindfourModel_Type) < 0)
    {
        goto error;
    }
    model_type = (PyObject *)&PymindfourModel_Type;
    Py_INCREF(model_type);

    if (PyType_Ready(&PymindfourTokenizer_Type) < 0)
    {
        goto error;
    }
    tokenizer_type = (PyObject *)&PymindfourTokenizer_Type;
    Py_INCREF(tokenizer_type);

    if (PyType_Ready(&PymindfourContext_Type) < 0)
    {
        goto error;
    }
    context_type = (PyObject *)&PymindfourContext_Type;
