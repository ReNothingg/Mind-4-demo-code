...
kernel void mindfour_f32_mf4w_moe_matmul_swiglu(
    constant mindfour_moe_matmul_swiglu_args& args [[ buffer(0) ]],
    const device float4* input [[ buffer(1) ]],
    const device mindfour_expert_prediction* expert [[ buffer(2) ]],
...
kernel void mindfour_f32_mf4w_moe_matmul(
    constant mindfour_moe_matmul_args& args [[ buffer(0) ]],
    const device float4* input [[ buffer(1) ]],
    const device mindfour_expert_prediction* expert [[ buffer(2) ]],
    const device uint4* weight_blocks [[ buffer(3) ]],
    const device uchar* weight_scales [[ buffer(4) ]],
