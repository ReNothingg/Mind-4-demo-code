...
static enum mindfour_status read_fd(int fd, void *data, size_t size, const char *path)
{
    assert(fd != -1);
    assert(data != NULL);
    assert(size != 0);
    ...
    do
    {
        ...
        if (read_result < 0)
        {
            mindfour_LOG_ERROR("reading %zu bytes from file %s failed with error %d",
                               size, path, errno);
            return mindfour_status_io_error;
        }
...
static void prefetch_fd(int fd, size_t offset, size_t size, const char *path)
{
    ...
    do
    {
        ...
        {
            mindfour_LOG_WARNING("fcntl(%s, F_RDADVISE, .ra_offset=%zu, .ra_count=%d) failed with error %d\n",
    ...
    mindfour_model_t *model_out)
{
    *model_out = NULL;
    ...
    struct mindfour_model *model = NULL;
    struct mindfour_tokenizer *tokenizer = NULL;
    int fd = -1;
    ...
    if (fd == -1)
    {
        mindfour_LOG_ERROR("open(%s) failed with error %d", path, errno);
        switch (errno)
        {
        case EISDIR:
        case ENOENT:
        case ENOTDIR:
            ...
            break;
        default:
            status = mindfour_status_io_error;
            break;
        }
        goto cleanup;
    }

    struct mindfour_file_header file_header;
    ...
    if (file_header.magic[0] != 'G' ||
        ...
        file_header.zero != 0)
    {
        mindfour_LOG_ERROR("invalid magic in file %s", path);
        status = mindfour_status_invalid_argument;
        goto cleanup;
    }

    struct mindfour_uuid model_uuid;
    ...
    if (!mindfour_is_mindfour_model_uuid(&model_uuid))
    {
        mindfour_LOG_ERROR("unsupported model UUID " UUID_FORMAT, UUID_ARGS(model_uuid));
        status = mindfour_status_invalid_argument;
        goto cleanup;
    }

    struct mindfour_mindfour_model_header model_header;
    ...
    struct mindfour_uuid layout_uuid;
    ...
    if (!mindfour_is_applegpu_layout_uuid(&layout_uuid))
    {
        mindfour_LOG_ERROR("unsupported layout UUID " UUID_FORMAT, UUID_ARGS(layout_uuid));
        status = mindfour_status_invalid_argument;
        goto cleanup;
    }

    const size_t model_size = sizeof(struct mindfour_model) + model_header.num_blocks * sizeof(struct mindfour_metal_buffer);
    model = malloc(model_size);
    if (model == NULL)
    {
        mindfour_LOG_ERROR("failed to allocate %zu bytes for model descriptor", model_size);
    ...
    model->max_batch_tokens = mindfour_DEFAULT_BATCH_SIZE;

    struct mindfour_uuid tokenizer_uuid;
    ...
    if (!mindfour_is_tiktoken_tokenizer_uuid(&tokenizer_uuid))
    {
        mindfour_LOG_ERROR("unsupported tokenizer UUID " UUID_FORMAT, UUID_ARGS(tokenizer_uuid));
        status = mindfour_status_invalid_argument;
        goto cleanup;
    }

    struct mindfour_tiktoken_tokenizer_header tokenizer_header;
    ...
    tokenizer = malloc(sizeof(struct mindfour_tokenizer));
    if (tokenizer == NULL)
    {
        mindfour_LOG_ERROR("failed to allocate %zu bytes for tokenizer descriptor", sizeof(struct mindfour_tokenizer));
        status = mindfour_status_insufficient_memory;
        goto cleanup;
    }
    memset(tokenizer, 0, sizeof(struct mindfour_tokenizer));
    ...
    {
        struct mindfour_uuid token_uuid;
        ...
        const enum mindfour_special_token token = mindfour_special_token_decode_uuid(&token_uuid);
        if (token != mindfour_special_token_invalid)
        {
    ...
    {
        mindfour_LOG_ERROR("failed to mmap(%s) tokenizer at offset %zu size %zu",
    ...
    {
        mindfour_LOG_WARNING("madvise(%s, size=%zu) failed with error %d", path, tokenizer_mapping_size, errno);
    }
    ...
    if (stat_result != 0)
    {
        mindfour_LOG_ERROR("stat(%s) failed with error %d", path, errno);
        status = mindfour_status_io_error;
        goto cleanup;
    }
    ...
    {
        mindfour_LOG_ERROR("failed to mmap(%s) model weights at offset %zu size %zu",
    ...
    {
        mindfour_LOG_WARNING("madvise(%s, size=%zu) failed with error %d", path, model_mapping_size, errno);
    }
    ...
    {
        mindfour_LOG_WARNING("mlock(%s, size=%zu) failed with error %d", path, model_mapping_size, errno);
    }
    else
    {
    ...
    status = mindfour_metal_device_create_system_default(&model->device);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    model->max_threadgroups = model->device.num_cores * 3;
    status = mindfour_metal_command_queue_create(&model->device, &model->command_queue);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }

    status = mindfour_metal_library_create_default(&model->device, &model->library);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_bf16_f32_embeddings", &model->bf16_f32_embeddings_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_bf16w_rmsnorm", &model->f32_bf16w_rmsnorm_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_bf16w_matmul", &model->f32_bf16w_matmul_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_bf16w_unembedding", &model->f32_bf16w_unembedding_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_rope", &model->f32_rope_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_mf4w_moe_matmul_swiglu", &model->f32_mf4w_moe_matmul_swiglu_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_mf4w_moe_matmul", &model->f32_mf4w_moe_matmul_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_accumulate_e4", &model->f32_accumulate_e4_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_topk_softmax_e32_k4", &model->f32_topk_softmax_e32_k4_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_topk_softmax_e128_k4", &model->f32_topk_softmax_e128_k4_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_softmax", &model->f32_softmax_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    status = mindfour_metal_function_create(&model->library, "mindfour_f32_sdpa_q8_d64", &model->f32_sdpa_q8_d64_fn);
    if (status != mindfour_status_success)
    {
        goto cleanup;
    }
    ...
    const size_t embedding_weight_size = math_round_up_po2(model->vocabulary_size * model->embedding_dim * sizeof(mindfour_bfloat16), 16);
    model->attn_rmsnorm_gain_offset = embedding_weight_size;
    const size_t rmsnorm_weight_size = math_round_up_po2(model->embedding_dim * sizeof(mindfour_bfloat16), 16);
    ...
    const size_t attn_qkv_weight_size = math_round_up_po2(attn_qkv_dim * model->embedding_dim * sizeof(mindfour_bfloat16), 16);
    model->attn_qkv_bias_offset = model->attn_qkv_weight_offset + attn_qkv_weight_size;
    const size_t attn_qkv_bias_size = math_round_up_po2(attn_qkv_dim * sizeof(mindfour_bfloat16), 16);
    model->attn_sdpa_sink_offset = model->attn_qkv_bias_offset + attn_qkv_bias_size;
    const size_t attn_sink_weight_size = math_round_up_po2(model->num_heads * sizeof(mindfour_bfloat16), 16);
    model->attn_out_weight_offset = model->attn_sdpa_sink_offset + attn_sink_weight_size;
    const size_t attn_out_weight_size = math_round_up_po2(model->embedding_dim * model->num_heads * model->head_dim * sizeof(mindfour_bfloat16), 16);
    model->attn_out_bias_offset = model->attn_out_weight_offset + attn_out_weight_size;
    const size_t attn_out_bias_size = math_round_up_po2(model->embedding_dim * sizeof(mindfour_bfloat16), 16);
    ...
    const size_t mlp_gate_weight_size = math_round_up_po2(model->num_experts * model->embedding_dim * sizeof(mindfour_bfloat16), 16);
    model->mlp_gate_bias_offset = model->mlp_gate_weight_offset + mlp_gate_weight_size;
    const size_t mlp_gate_bias_size = math_round_up_po2(model->num_experts * sizeof(mindfour_bfloat16), 16);
    ...
    const size_t unembedding_weight_size = math_round_up_po2(model->vocabulary_size * model->embedding_dim * sizeof(mindfour_bfloat16), 16);
    ...
    status = mindfour_metal_buffer_wrap(&model->device, shared_weights_size, current_ptr, &model->shared_weight_buffer);
    if (status != mindfour_status_success)
    {
        mindfour_LOG_ERROR("failed to map expert-shared weight of size %zu onto a Metal buffer", shared_weights_size);
        goto cleanup;
    }
    ...
    const size_t mlp_swiglu_bias_size = math_round_up_po2(2 * model->mlp_dim * sizeof(mindfour_bfloat16), 16);
    ...
    const size_t mlp_out_bias_size = math_round_up_po2(model->embedding_dim * sizeof(mindfour_bfloat16), 16);
    ...
    {
        status = mindfour_metal_buffer_wrap(&model->device, moe_block_weight_size, current_ptr, &model->block_weight_buffers[n]);
        if (status != mindfour_status_success)
        {
            mindfour_LOG_ERROR("failed to map block #%" PRIu32 " MoE weight of size %zu onto a Metal buffer",
...
cleanup:
    ...
    mindfour_model_release(model);
    mindfour_tokenizer_release(tokenizer);
    return status;
}
...
{
    mindfour_tokenizer_t tokenizer = model->tokenizer;
    ...
    mindfour_model_t model)
{
    if (model != NULL)
    {
        ...
        {
            mindfour_tokenizer_release(model->tokenizer);

            mindfour_metal_buffer_release(&model->shared_weight_buffer);
            for (uint32_t n = 0; n < model->num_blocks; n++)
            {
                mindfour_metal_buffer_release(&model->block_weight_buffers[n]);
            }

            mindfour_metal_function_release(&model->bf16_f32_embeddings_fn);
            mindfour_metal_function_release(&model->f32_bf16w_rmsnorm_fn);
            mindfour_metal_function_release(&model->f32_bf16w_matmul_fn);
            mindfour_metal_function_release(&model->f32_bf16w_unembedding_fn);
            mindfour_metal_function_release(&model->f32_rope_fn);
            mindfour_metal_function_release(&model->f32_mf4w_moe_matmul_swiglu_fn);
            mindfour_metal_function_release(&model->f32_mf4w_moe_matmul_fn);
            mindfour_metal_function_release(&model->f32_accumulate_e4_fn);
            mindfour_metal_function_release(&model->f32_topk_softmax_e32_k4_fn);
            mindfour_metal_function_release(&model->f32_topk_softmax_e128_k4_fn);
            mindfour_metal_function_release(&model->f32_softmax_fn);
            mindfour_metal_function_release(&model->f32_sdpa_q8_d64_fn);
            mindfour_metal_library_release(&model->library);

            mindfour_metal_command_queue_release(&model->command_queue);
            mindfour_metal_device_release(&model->device);
            ...
            if (model->mapping_ptr != NULL && model->mapping_size != 0)
            {
                if (model->lock_memory)
                {
                    ...
                    {
                        mindfour_LOG_WARNING("munlock for model weight mapping failed with error %d", errno);
                    }
                }

                if (munmap(model->mapping_ptr, model->mapping_size) != 0)
                {
                    mindfour_LOG_WARNING("munmap for model weight mapping failed with error %d", errno);
                }
            }

            const size_t model_size = sizeof(struct mindfour_model) + model->num_blocks * sizeof(struct mindfour_metal_buffer);
            memset(model, 0, model_size);
            free(model);
        }
    }
