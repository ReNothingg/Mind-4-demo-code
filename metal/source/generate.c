...
int main(int argc, char *argv[])
{
    ...
    mindfour_model_t model = NULL;
    mindfour_tokenizer_t tokenizer = NULL;
    mindfour_context_t context = NULL;
    ...
    status = mindfour_model_create_from_file(options.model, &model);
    ...
    status = mindfour_model_get_max_context_length(model, &max_model_context_length);
    if (status != mindfour_status_success)
    {
    ...
    status = mindfour_model_get_tokenizer(model, &tokenizer);
    if (status != mindfour_status_success)
    {
    ...
    status = mindfour_tokenizer_get_special_token_id(tokenizer, mindfour_special_token_return, &return_token_id);
    if (status != mindfour_status_success)
    {
    ...
    status = mindfour_context_create(model, options.context_length, &context);
    if (status != mindfour_status_success)
    {
    ...
    status = mindfour_context_append_chars(context, options.prompt, strlen(options.prompt), &num_prefill_tokens);
    if (status != mindfour_status_success)
    {
    ...
    status = mindfour_context_process(context);
    if (status != mindfour_status_success)
    {
    ...
    while (options.max_tokens == 0 || atomic_load(&globals.num_generated_tokens) < options.max_tokens)
    {
        ...
        status = mindfour_context_sample(context, options.temperature, 0, &predicted_token);
        if (status != mindfour_status_success)
        {
        ...
        status = mindfour_tokenizer_decode(tokenizer, predicted_token, &token_ptr, &token_size);
        if (status != mindfour_status_success)
        {
        ...
        status = mindfour_context_append_tokens(context, 1, &predicted_token);
        if (status != mindfour_status_success)
        {
