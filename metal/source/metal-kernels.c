...
enum mindfour_status mindfour_metal_command_buffer_encode_launch_u32_fill_random(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *u32_fill_random_fn,
    ...
    uint64_t rng_offset)
{
    ...
    {
        return mindfour_status_invalid_state;
    }
    ...
    const struct mindfour_u32_fill_random_args args = {
        .num_vecs = num_vecs,
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
...
enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_fill_random(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_fill_random_fn,
    size_t threadgroup_size,
    size_t max_threadgroups,
    const struct mindfour_metal_buffer *output_buffer,
    size_t output_offset,
    ...
    float rng_max)
{
    ...
    const size_t num_vecs_per_threadgroup = math_ceil_div(num_vecs, max_threadgroups * threadgroup_size) * threadgroup_size;
    const size_t num_threadgroups = math_min(max_threadgroups, math_ceil_div(num_vecs, num_vecs_per_threadgroup));
    const struct mindfour_f32_fill_random_args args = {
        .num_vecs = num_vecs,
        .num_vecs_per_threadgroup = num_vecs_per_threadgroup,
        .seed = rng_seed,
        .offset = rng_offset,
...
enum mindfour_status mindfour_metal_command_buffer_encode_launch_bf16_fill_random(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *bf16_fill_random_fn,
    size_t threadgroup_size,
    size_t max_threadgroups,
    const struct mindfour_metal_buffer *output_buffer,
    size_t output_offset,
...
enum mindfour_status mindfour_metal_command_buffer_encode_launch_mf4_f32_convert(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *mf4_f32_convert_fn,
    size_t threadgroup_size,
    size_t max_threadgroups,
    const struct mindfour_metal_buffer *block_buffer,
    const struct mindfour_metal_buffer *scale_buffer,
    const struct mindfour_metal_buffer *output_buffer,
    uint64_t num_elements)
{
    ...
    const size_t num_vecs_per_threadgroup = math_ceil_div(num_vecs, max_threadgroups * threadgroup_size) * threadgroup_size;
    const size_t num_threadgroups = math_min(max_threadgroups, math_ceil_div(num_vecs, num_vecs_per_threadgroup));
    const struct mindfour_convert_args args = {
        .num_vecs = num_vecs,
        .num_vecs_per_threadgroup = num_vecs_per_threadgroup,
    };
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        num_threadgroups, 1, 1,
        sizeof(args), &args,
        3, (const struct mindfour_metal_buffer *[]){block_buffer, scale_buffer, output_buffer}, NULL,
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_bf16_f32_embeddings(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *bf16_f32_embeddings_fn,
    size_t threadgroup_size,
    const struct mindfour_metal_buffer *token_buffer,
    size_t token_offset,
    const struct mindfour_metal_buffer *weight_buffer,
    ...
    uint32_t num_channels)
{
    ...
    const struct mindfour_embeddings_args args = {
        .num_vecs = num_vecs,
    };
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        3,
        (const struct mindfour_metal_buffer *[]){token_buffer, weight_buffer, output_buffer},
        (const size_t[]){token_offset, weight_offset, output_offset},
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_bf16w_rmsnorm(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_bf16w_rmsnorm_fn,
    ...
    float epsilon)
{
    ...
    {
        return mindfour_status_unsupported_system;
    }
    ...
    const uint32_t num_vecs = num_channels / 4;
    const struct mindfour_rmsnorm_args args = {
        .num_vecs = num_vecs,
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        3,
        (const struct mindfour_metal_buffer *[]){input_buffer, weight_buffer, output_buffer},
        (const size_t[]){input_offset, weight_offset, output_offset},
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_bf16w_matmul(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_bf16w_matmul_fn,
    size_t threadgroup_size,
    const struct mindfour_metal_buffer *input_buffer,
    size_t input_offset,
    const struct mindfour_metal_buffer *weight_buffer,
    size_t weight_offset,
    const struct mindfour_metal_buffer *bias_buffer,
    size_t bias_offset,
    const struct mindfour_metal_buffer *output_buffer,
    size_t output_offset,
    uint32_t num_tokens,
    ...
    uint32_t num_rows)
{
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_matmul kernel launch: invalid command buffer or pipeline state object");
        return mindfour_status_invalid_state;
    }
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_matmul kernel launch: threadgroup size (%zu) exceeds supported maximum (%zu)",
    ...
    if (num_cols % 4 != 0)
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_matmul kernel launch: number of columns (%" PRIu32 ") is not divisible by 4",
                           num_cols);
        return mindfour_status_invalid_argument;
    }
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_matmul kernel launch: number of rows (%" PRIu32 ") is not divisible by the number of simdgroups (%zu)",
                           num_rows, num_simdgroups);
        return mindfour_status_invalid_argument;
    }

    const struct mindfour_matmul_args args = {
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        4,
        (const struct mindfour_metal_buffer *[]){input_buffer, weight_buffer, bias_buffer, output_buffer},
        (const size_t[]){input_offset, weight_offset, bias_offset, output_offset},
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_bf16w_matmul_add(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_bf16w_matmul_fn,
    size_t threadgroup_size,
    const struct mindfour_metal_buffer *input_buffer,
    ...
    uint32_t num_rows)
{
    if (command_buffer->object == NULL || f32_bf16w_matmul_fn->pipeline_state_object == NULL)
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_matmul_add kernel launch: invalid command buffer or pipeline state object");
        return mindfour_status_invalid_state;
    }
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_matmul_add kernel launch: threadgroup size (%zu) exceeds supported maximum (%zu)",
                           threadgroup_size, f32_bf16w_matmul_fn->max_threadgroup_threads);
        return mindfour_status_invalid_argument;
    }
    ...
    if (num_cols % 4 != 0)
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_matmul_add kernel launch: number of columns (%" PRIu32 ") is not divisible by 4",
                           num_cols);
        return mindfour_status_invalid_argument;
    }
    const size_t num_simdgroups = threadgroup_size / f32_bf16w_matmul_fn->simdgroup_threads;
    if (num_rows % num_simdgroups != 0)
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_matmul_add kernel launch: number of rows (%" PRIu32 ") is not divisible by the number of simdgroups (%zu)",
                           num_rows, num_simdgroups);
        return mindfour_status_invalid_argument;
    }
...
enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_bf16w_unembedding(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_bf16w_unembedding_fn,
    size_t threadgroup_size,
    size_t max_threadgroups,
    const struct mindfour_metal_buffer *input_buffer,
    size_t input_offset,
    ...
    const struct mindfour_metal_buffer *output_buffer,
    size_t output_offset,
    const struct mindfour_metal_buffer *argmax_buffer,
    ...
    uint32_t num_rows)
{
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_unembedding kernel launch: invalid command buffer or pipeline state object");
        return mindfour_status_invalid_state;
    }
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_unembedding kernel launch: threadgroup size (%zu) exceeds supported maximum (%zu)",
    ...
    if (num_cols % 4 != 0)
    {
        mindfour_LOG_ERROR("failed to encode f32_bf16w_unembedding kernel launch: number of columns (%" PRIu32 ") is not divisible by 4",
                           num_cols);
        return mindfour_status_invalid_argument;
    }
    ...
    const struct mindfour_unembedding_args args = {
        .num_column_vecs = num_cols / 4,
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        4,
        (const struct mindfour_metal_buffer *[]){input_buffer, weight_buffer, output_buffer, argmax_buffer},
        (const size_t[]){input_offset, weight_offset, output_offset, argmax_offset},
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_mf4w_moe_matmul_swiglu(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_mf4w_moe_matmul_swiglu_fn,
    size_t threadgroup_size,
    const struct mindfour_metal_buffer *input_buffer,
    size_t input_offset,
    const struct mindfour_metal_buffer *expert_buffer,
    size_t expert_offset,
    const struct mindfour_metal_buffer *weight_block_buffer,
    size_t weight_block_offset,
    const struct mindfour_metal_buffer *weight_scale_buffer,
    ...
    uint32_t num_rows)
{
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_mf4w_moe_matmul_swiglu kernel launch: invalid command buffer or pipeline state object");
        return mindfour_status_invalid_state;
    }
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_mf4w_moe_matmul_swiglu kernel launch: threadgroup size (%zu) exceeds supported maximum (%zu)",
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_mf4w_moe_matmul_swiglu kernel launch: threadgroup size (%zu) is not divisible by simdgroup size (%zu) multiplied by 2X",
    ...
    if (num_cols % 32 != 0)
    {
        mindfour_LOG_ERROR("failed to encode f32_mf4w_moe_matmul_swiglu kernel launch: number of columns (%" PRIu32 ") is not divisible by 32",
                           num_cols);
        return mindfour_status_invalid_argument;
    }
    ...
    const struct mindfour_moe_matmul_swiglu_args args = {
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        6,
        (const struct mindfour_metal_buffer *[]){input_buffer, expert_buffer, weight_block_buffer, weight_scale_buffer, bias_buffer, output_buffer},
        (const size_t[]){input_offset, expert_offset, weight_block_offset, weight_scale_offset, bias_offset, output_offset},
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_mf4w_moe_matmul(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_mf4w_moe_matmul_fn,
    size_t threadgroup_size,
    const struct mindfour_metal_buffer *input_buffer,
    size_t input_offset,
    const struct mindfour_metal_buffer *expert_buffer,
    ...
    uint32_t num_rows)
{
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_mf4w_moe_matmul kernel launch: invalid command buffer or pipeline state object");
        return mindfour_status_invalid_state;
    }
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_mf4w_moe_matmul kernel launch: threadgroup size (%zu) exceeds supported maximum (%zu)",
    ...
    {
        mindfour_LOG_ERROR("failed to encode f32_mf4w_moe_matmul kernel launch: threadgroup size (%zu) is not divisible by simdgroup size (%zu)",
                           threadgroup_size, f32_mf4w_moe_matmul_fn->simdgroup_threads);
        return mindfour_status_invalid_argument;
    }
    ...
    if (num_cols % 32 != 0)
    {
        mindfour_LOG_ERROR("failed to encode f32_mf4w_moe_matmul kernel launch: number of columns (%" PRIu32 ") is not divisible by 32",
                           num_cols);
        return mindfour_status_invalid_argument;
    }
    ...
    const struct mindfour_moe_matmul_args args = {
        .num_column_vecs = num_cols / 32,
        .num_rows = num_rows,
        .num_active_experts = num_active_experts,
...
enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_rope(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_rope_fn,
    size_t threadgroup_size,
    const struct mindfour_metal_buffer *activations_buffer,
    float rope_base,
    ...
    uint32_t token_offset)
{
    ...
    const struct mindfour_rope_args args = {
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        1, (const struct mindfour_metal_buffer *[]){activations_buffer}, NULL,
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_accumulate(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_accumulate_fn,
    size_t threadgroup_size,
    size_t max_threadgroups,
    const struct mindfour_metal_buffer *input_buffer,
    size_t input_offset,
    ...
    uint32_t num_experts)
{
    ...
    const size_t num_vecs_per_threadgroup = math_ceil_div(num_vecs, max_threadgroups * threadgroup_size) * threadgroup_size;
    const size_t num_threadgroups = math_min(max_threadgroups, math_ceil_div(num_vecs, num_vecs_per_threadgroup));
    const struct mindfour_accumulate_args args = {
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        3,
        (const struct mindfour_metal_buffer *[]){input_buffer, expert_buffer, output_buffer},
        (const size_t[]){input_offset, expert_offset, output_offset},
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_topk(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_topk_fn,
    const struct mindfour_metal_buffer *input_buffer,
    size_t input_offset,
    const struct mindfour_metal_buffer *output_buffer,
    size_t output_offset,
    ...
    uint32_t num_active_experts)
{
    ...
    const struct mindfour_topk_args args = {0};
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        2,
        (const struct mindfour_metal_buffer *[]){input_buffer, output_buffer},
        (const size_t[]){input_offset, output_offset},
        0);
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_sdpa(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_sdpa_fn,
    const struct mindfour_metal_buffer *q_buffer,
    size_t q_offset,
    const struct mindfour_metal_buffer *k_buffer,
    size_t k_offset,
    const struct mindfour_metal_buffer *v_buffer,
    size_t v_offset,
    const struct mindfour_metal_buffer *s_buffer,
    size_t s_offset,
    const struct mindfour_metal_buffer *output_buffer,
    size_t output_offset,
    uint32_t window,
    ...
    uint32_t head_dim)
{
    ...
    {
        mindfour_LOG_ERROR("number of Q heads (%" PRIu32 ") must be 8 times the number of KV heads (%" PRIu32 ")",
    ...
    if (head_dim != 64)
    {
        mindfour_LOG_ERROR("attention head dimension (%" PRIu32 ") must be 64", head_dim);
        return mindfour_status_invalid_argument;
    }
    ...
    const struct mindfour_sdpa_args args = {
    ...
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        5,
        (const struct mindfour_metal_buffer *[]){q_buffer, k_buffer, v_buffer, s_buffer, output_buffer},
        (const size_t[]){q_offset, k_offset, v_offset, s_offset, output_offset},
        half_threadgroup_size * 8 * 4 * sizeof(float));
}

enum mindfour_status mindfour_metal_command_buffer_encode_launch_f32_softmax(
    const struct mindfour_metal_command_buffer *command_buffer,
    const struct mindfour_metal_function *f32_softmax_fn,
    size_t threadgroup_size,
    size_t max_threadgroups,
    const struct mindfour_metal_buffer *score_buffer,
    size_t score_offset,
    const struct mindfour_metal_buffer *argmax_buffer,
    size_t argmax_offset,
    const struct mindfour_metal_buffer *prob_buffer,
    size_t prob_offset,
    const struct mindfour_metal_buffer *sum_buffer,
    size_t sum_offset,
    uint32_t num_channels,
    uint32_t num_tokens,
    float temperature,
    ...
    uint32_t *num_channels_per_threadgroup_out)
{
    ...
    *num_channels_per_threadgroup_out = 0;
    ...
    const size_t num_vecs_per_threadgroup = math_ceil_div(num_vecs, max_threadgroups * threadgroup_size) * threadgroup_size;
    const size_t num_threadgroups = math_min(max_threadgroups, math_ceil_div(num_vecs, num_vecs_per_threadgroup));
    const struct mindfour_softmax_args args = {
        .num_vecs = num_vecs,
        .num_vecs_per_threadgroup = num_vecs_per_threadgroup,
    ...
    *num_channels_per_threadgroup_out = num_vecs_per_threadgroup;
    return mindfour_metal_command_buffer_encode_launch_kernel(
        ...
        sizeof(args), &args,
        4,
        (const struct mindfour_metal_buffer *[]){score_buffer, argmax_buffer, prob_buffer, sum_buffer},
