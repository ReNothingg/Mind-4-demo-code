    ...
    uint32_t *token_id_out)
{
    ...
    if (token_type != mindfour_special_token_invalid && token_type < mindfour_special_token_max)
    {
    ...
    if (tokenizer != NULL)
    {
        if (atomic_fetch_sub_explicit(&tokenizer->ref_count, 1, memory_order_acquire) == 1)
        {
            if (tokenizer->mapping_ptr != NULL && tokenizer->mapping_size != 0)
            {
                ...
                {
                    mindfour_LOG_WARNING("munmap for tokenizer mapping failed with error %d", errno);
                }
            }
